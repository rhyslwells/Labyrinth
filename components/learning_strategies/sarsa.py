...
class Sarsa:
    ...
    def update(self, s_t, a_t, reward, s_next, a_next):
        self.q_table[tuple(s_t)][a_t] += self.learning_rate * self._get_temporal_difference(s_t, a_t, reward, s_next, a_next)

    def _get_temporal_difference(self, s_t, a_t, reward, s_next, a_next):
        return self._get_temporal_target(reward, s_next, a_next) - self.q_table[tuple(s_t)][a_t]
